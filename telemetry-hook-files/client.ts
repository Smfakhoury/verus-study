...
import { telemetry, getFilename } from "./commands";
    ...
    const clientOptions: lc.LanguageClientOptions = {
        ...
        middleware: {
            ...
            async handleDiagnostics(
                ...
                const starts = diagnosticList.map(
                    s => `${s.range.start.line}:${s.range.start.character}`).join('\n');
                const ends = diagnosticList.map(
                    e => `${e.range.end.line}:${e.range.end.character}`).join('\n');
                const messages = diagnosticList.map(
                    m => `${m.message}`).join('\n');
                telemetry.sendRawTelemetryEvent('verusDiagnostics', {fileName: getFilename(uri.toString()), 
                    contents: vscode.workspace.textDocuments.filter(e => e.uri.toString() === uri.toString())[0]?.getText(), starts: starts, ends: ends, messages: messages,
                });
                ...
                return client
                    ...
                    .then(
                        (result) => {
                            ...
                            // hover hook
                            const hoverContents = hover.contents.map(
                                e => `${e}`).join('\n');
                            telemetry.sendRawTelemetryEvent('verusHover', {fileName: getFilename(document.uri.toString()),
                                contents: document.getText(), hover: hoverContents, line: position.line.toString(),
                                character: position.character.toString(),
                            });
                            return hover;
                        },
                        (error) => {
