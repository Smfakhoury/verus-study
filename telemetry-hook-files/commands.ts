...
/*
    The following SignalProc and RateLimiter are taken from
    https://github.com/FStarLang/fstar-vscode-assistant/server/src/signals.ts
*/
interface SignalProc {
	fire(): void;
	handler: () => void;
}
...
const changeRateLimiter = new RateLimiter(2000, () => {
...
export function runFlycheck(ctx: CtxInit): Cmd {
    return async () => {
        ...
        telemetry.sendRawTelemetryEvent('verusRunFlycheck', {fileName: getFilename(uri), contents: editor!.document.getText(),
        });
